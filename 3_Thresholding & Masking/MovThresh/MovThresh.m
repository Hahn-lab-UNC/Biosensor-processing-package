...
% Last Modified by GUIDE v2.5 18-Feb-2017 13:05:37
...
% hlismove2=event.listener(handles.movieSlider, 'PostSet', @(src, event)replot(hObject, src, event));
% handles.lisdraw1=event.listener(handles.drawerSlider, 'PostSet', @(src, event)redrawMean(hObject, src, event));
% hlisdraw5=event.listener(handles.drawerSlider, 'PostSet', @(src, event)translateSmooth(hObject, src, event));
% hlisdraw2=event.listener(handles.drawerSlider, 'PostSet', @(src, event)readjustFrame(hObject, src, event));
...
% optimal intensities for every time unit.
    ...
    value = round(event.AffectedObject.Value);
    ...
    handles.REDLINE = line([value, value], get(handles.histogramLoc, 'YLim'), 'Color', 'r', 'Parent',handles.histogramLoc);
    %handles.REDLINE = animatedline([value, value], get(handles.histogramLoc, 'YLim'), 'Color', 'r', 'Parent',handles.histogramLoc);
    %, 'EraseMode', 'normal'); 
...
%         set(handles.lisdraw5, 'Enable', 'off');
%         handles.lisdraw1.Enabled = false;
%         handles.lisdraw2.Enabled = false;
%         handles.lisdraw5.Enabled = false;
...
%         set(handles.lisdraw5, 'Enable', 'on');
%         handles.lisdraw1.Enabled = true;
%         handles.lisdraw2.Enabled = true;
%         handles.lisdraw5.Enabled = true;
...
% the threshold point of one frame.   
    ...
    newIntensity = event.AffectedObject.Value;
...
function translateSmooth(hObject, src, event)
    ...
    if (newValue ~= handles.gaussianSavedSliderValue)
        ...
        axes(handles.histogramLoc);
        handles.CUST_LINE = line('Xdata', X, 'YData', Y, 'Color', 'r','Marker','.');
        %'EraseMode', 'normal',
        ...
        if filename~=0
            ...
            if handles.NUM_ELEMENTS>1
                ...
                if ~ishandle(handles.REDLINE) || handles.REDLINE==0
                    handles.REDLINE = line([1, 1], get(handles.histogramLoc, 'YLim'), 'Color', 'r', 'Parent',handles.histogramLoc);
                    %, 'EraseMode', 'normal'); 
                end
...
%                 handles.lisdraw2.Enabled = 'on';
%                 handles.lisdraw5.Enabled = 'on';
%                 handles.lismove1.Enabled = true;
%                 handles.lismove2.Enabled = true;
%                 handles.lisdraw1.Enabled = true;
%                 handles.lisdraw2.Enabled = true;
%                 handles.lisdraw5.Enabled = true;                
...
% frame within the intensity range specified by the user.
    ...
    try
        if handles.NUM_ELEMENTS>1
            ...
            str=sprintf('Threshold = %.2f',get(handles.drawerSlider,'Value'));
            set(handles.threshInt, 'String', str);
            axes(handles.imageLoc);
            xlim=get(handles.imageLoc,'XLim');
            ylim=get(handles.imageLoc,'YLim');
...
% user to change the curve fit on the intensity plot. 
    ...
    switch get(eventdata.NewValue, 'Tag')
        case 'gaussian'
            ...
            handles.GAUSSIAN = Y;
            handles.CUST_LINE = line('Xdata', X, 'YData', Y, 'Color', 'r', 'Marker','.', 'Parent', handles.histogramLoc); 
            %'EraseMode', 'normal',
        ...
        case 'custom'
            ...
            set(handles.drawerSlider, 'Value', handles.CUST_VALUES(handles.IMAGE_NUMBER));
            %replot();
            handles.CUST_LINE = line('Xdata', handles.CUST_X, 'YData', handles.CUST_Y, 'Color', 'k', 'Marker','o','MarkerFaceColor','r','MarkerSize',4, 'Parent',handles.histogramLoc); 
            % 'EraseMode', 'normal',
            str=sprintf('Threshold = %.2f',handles.CUST_VALUES(handles.IMAGE_NUMBER));
            set(handles.threshInt, 'String', str);
            set(handles.drawerSlider, 'Enable', 'on');
...
function algo_Callback(hObject, eventdata, handles)
    ...
    if handles.NUM_ELEMENTS > 1
        ...
        if ~ishandle(handles.REDLINE) || handles.REDLINE==0            
            handles.REDLINE = animatedline([1, 1], get(handles.histogramLoc, 'YLim'), 'Color', 'r',...
                'Parent', handles.histogramLoc);%, 'EraseMode', 'normal'); 
        end
            ...
            if handles.NUM_ELEMENTS>1
                ...
                if ~isempty(up) && ~isempty(lw) 
                    if up>=ma && lw<=mi 
                        ...
                        if ishandle(handles.REDLINE) && handles.REDLINE ~= 0
                            delete(handles.REDLINE);
                            handles.REDLINE = line([handles.IMAGE_NUMBER, handles.IMAGE_NUMBER], get(handles.histogramLoc, 'YLim'), 'Color', 'r',...
                                'Parent',handles.histogramLoc);
                                %, 'EraseMode', 'normal'); 
                        end
                    else
