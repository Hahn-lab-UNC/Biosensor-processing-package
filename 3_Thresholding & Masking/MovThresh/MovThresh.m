...
handles.lisdraw7=addlistener(handles.drawerSlider, 'Value', 'PostSet', @(src, event)currentSliderVal(hObject, src, event));
...
function currentSliderVal(hObject, src, event)
handles = guidata(hObject);
str = sprintf('Slider Value = %.2f',get(handles.drawerSlider,'Value'));
set(handles.sliderVal,'String',str);
guidata(hObject,handles);
% -------------------------------------------------------------------------
...
[filename,pathname] = uigetfile('*.tif','Select a ".tif" file to open for applying mask'); 
if filename~=0
...
elseif handles.NUM_ELEMENTS==1
    ...
    b{1} = thresholdByValue(hObject,image,y);                
